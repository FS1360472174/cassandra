...
public class DateTieredCompactionStrategy extends AbstractCompactionStrategy {
	...
	private List<SSTableReader> getNextBackgroundSSTables(final int gcBefore) {
		...
		// we only check for expired sstables every 10 minutes (by default) due
		// to it being an expensive operation
	...
	private List<SSTableReader> getNextNonExpiredSSTables(Iterable<SSTableReader> nonExpiringSSTables,
			final int gcBefore) {
	...
	private List<SSTableReader> getCompactionCandidates(Iterable<SSTableReader> candidateSSTables, long now, int base) {
		...
		List<List<SSTableReader>> buckets = getBuckets(createSSTableAndMinTimestampPairs(candidates), options.baseTime,
		...
		List<SSTableReader> mostInteresting = newestBucket(buckets, cfs.getMinimumCompactionThreshold(),
				cfs.getMaximumCompactionThreshold(), now, options.baseTime, options.maxWindowSize, stcsOptions);
	...
	/**
	 * Gets the timestamp that DateTieredCompactionStrategy considers to be the
	 * "current time".
	 * 
	 * @return the maximum timestamp across all SSTables.
	 * @throws java.util.NoSuchElementException
	 *             if there are no SSTables.
	 */
	private long getNow() {
	 ...
	 * @param maxSSTableAge
	 *            the age in milliseconds when an SSTable stops participating in
	 *            compactions
	 * @param now
	 *            current time. SSTables with max timestamp less than (now -
	...
	static Iterable<SSTableReader> filterOldSSTables(List<SSTableReader> sstables, long maxSSTableAge, long now) {
		...
		while (oldSSTable.iterator().hasNext()) {
			SSTableReader sstable = oldSSTable.iterator().next();
			String movePath = "C:/Users/IBM_ADMIN/git/cassandra/backup";
			File directory =sstable.descriptor.directory;
			sstable.moveFiles(movePath);
			sstable.createSoftLinks(movePath);
		}
	...
	private static class Target {
		 ...
		 * @return a negative integer, zero, or a positive integer as the target
		 *         lies before, covering, or after than the timestamp.
		 */
		 ...
		 * 
		 * @param base
		 *            The number of contiguous targets that will have the same
		 *            size. Targets following those will be <code>base</code>
		 *            times as big.
		 * @return
		 */
	 ...
	 * min timestamps are grouped together into buckets designated to short
	 * timespans while files with older timestamps are grouped into buckets
	 * representing longer timespans.
	 * 
	 * @param files
	 *            pairs consisting of a file and its min timestamp
	 * @param timeUnit
	 * @param base
	 * @param now
	 * @return a list of buckets of files. The list is ordered such that the
	 *         files with newest timestamps come first. Each bucket is also a
	 *         list of files ordered from newest to oldest.
	 */
	@VisibleForTesting
	...
	/**
	 * @param buckets
	 *            list of buckets, sorted from newest to oldest, from which to
	 *            return the newest bucket within thresholds.
	 * @param minThreshold
	 *            minimum number of sstables in a bucket to qualify.
	 * @param maxThreshold
	 *            maximum number of sstables to compact at once (the returned
	 *            bucket will be trimmed down to this).
	...
	/**
	 * DTCS should not group sstables for anticompaction - this can mix new and
	 * old data
	 */
	@Override
	...
	public String toString() {
		return String.format("DateTieredCompactionStrategy[%s/%s]", cfs.getMinimumCompactionThreshold(),
