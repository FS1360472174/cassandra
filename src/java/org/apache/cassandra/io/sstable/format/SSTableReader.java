...
/**
 * An SSTableReader can be constructed in a number of places, but typically is
 * either read from disk at startup, or constructed from a flushed memtable, or
 * after compaction to replace some existing sstables. However once created, an
 * sstablereader may also be modified.
 *
 ...
 * </pre>
 *
 * Note that in parallel to this, there are two different Descriptor types;
 * TMPLINK and FINAL; the latter corresponds to NORMAL state readers and all
 * readers that replace a NORMAL one. TMPLINK is used for EARLY state readers
 * and no others.
 *
 * When a reader is being compacted, if the result is large its replacement may
 * be opened as EARLY before compaction completes in order to present the result
 * to consumers earlier. In this case the reader will itself be changed to a
 * MOVED_START state, where its start no longer represents its on-disk minimum
 * key. This is to permit reads to be directed to only one reader when the two
 * represent the same data. The EARLY file can represent a compaction result
 * that is either partially complete and still in-progress, or a complete and
 * immutable sstable that is part of a larger macro compaction action that has
 * not yet fully completed.
 *
 * Currently ALL compaction results at least briefly go through an EARLY open
 * state prior to completion, regardless of if early opening is enabled.
 *
 * Since a reader can be created multiple times over the same shared underlying
 * resources, and the exact resources it shares between each instance differ
 * subtly, we track the lifetime of any underlying resource with its own
 * reference count, which each instance takes a Ref to. Each instance then
 * tracks references to itself, and once these all expire it releases its Refs
 * to these underlying resources.
 *
 * There is some shared cleanup behaviour needed only once all sstablereaders in
 * a certain stage of their lifecycle (i.e. EARLY or NORMAL opening), and some
 * that must only occur once all readers of any kind over a single logical
 * sstable have expired. These are managed by the TypeTidy and GlobalTidy
 * classes at the bottom, and are effectively managed as another resource each
 * instance tracks its own Ref instance to, to ensure all of these resources are
 * cleaned up safely and can be debugged otherwise.
 *
 * TODO: fill in details about Tracker and lifecycle interactions for tools, and
 * for compaction strategies
 */
<<<<<<< HEAD
public abstract class SSTableReader extends SSTable implements SelfRefCounted<SSTableReader> {
	 ...
	 * System.currentTimeMilli) which represents an upper bound to the newest
	 * piece of data stored in the sstable. In other words, this sstable does
	 * not contain items created later than maxDataAge.
	 *
	 * The field is not serialized to disk, so relying on it for more than what
	 * truncate does is not advised.
	 *
	...
	/**
	 * Calculate approximate key count. If cardinality estimator is available on
	 * all given sstables, then this method use them to estimate key count. If
	 * not, then this uses index summaries.
	 *
	 * @param sstables
	...
	public static long getApproximateKeyCount(Iterable<SSTableReader> sstables) {
		...
		boolean cardinalityAvailable = !Iterables.isEmpty(sstables)
				&& Iterables.all(sstables, new Predicate<SSTableReader>() {
		...
		if (cardinalityAvailable) {
			...
			for (SSTableReader sstable : sstables) {
				...
				try {
					...
					// If we can't load the CompactionMetadata, we are forced to
					// estimate the keys using the index
	...
	/**
	 * Estimates how much of the keys we would keep if the sstables were
	 * compacted together
	 */
	public static double estimateCompactionGain(Set<SSTableReader> overlapping) {
		...
		for (SSTableReader sstable : overlapping) {
			try {
				ICardinality cardinality = ((CompactionMetadata) sstable.descriptor.getMetadataSerializer()
						.deserialize(sstable.descriptor, MetadataType.COMPACTION)).cardinalityEstimator;
				if (cardinality != null)
		...
		logger.trace("Estimated compaction gain: {}/{}={}", totalKeyCountAfter, totalKeyCountBefore,
				((double) totalKeyCountAfter) / totalKeyCountBefore);
	...
	private static ICardinality mergeCardinalities(Collection<ICardinality> cardinalities) {
		ICardinality base = new HyperLogLogPlus(13, 25); // see
															// MetadataCollector.cardinality
		try {
	...
	public static SSTableReader open(Descriptor descriptor) throws IOException {
		...
		if (descriptor.cfname.contains(SECONDARY_INDEX_NAME_SEPARATOR)) {
			...
			IndexMetadata def = parent.getIndexes().get(indexName)
	...
	public static SSTableReader open(Descriptor descriptor, Set<Component> components, CFMetaData metadata)
			throws IOException {
		return open(descriptor, components, metadata, true, true);
	}

	// use only for offline or "Standalone" operations
	public static SSTableReader openNoValidation(Descriptor descriptor, Set<Component> components,
	...
	public static SSTableReader openNoValidation(Descriptor descriptor, CFMetaData metadata) throws IOException {
		return open(descriptor, componentsFor(descriptor), metadata, false, false); // do
																					// not
																					// track
																					// hotness
	}
	...
	public static SSTableReader openForBatch(Descriptor descriptor, Set<Component> components, CFMetaData metadata)
		...
		assert components.contains(Component.PRIMARY_INDEX) : "Primary index component is missing for sstable "
				+ descriptor;
		...
		// Partitioner can be null, which indicates older version of sstable or
		// no stats available.
		...
		SSTableReader sstable = internalOpen(descriptor, components, metadata, System.currentTimeMillis(),
				statsMetadata, OpenReason.NORMAL, header.toHeader(metadata));
		...
		try (SegmentedFile.Builder ibuilder = new BufferedSegmentedFile.Builder();
				SegmentedFile.Builder dbuilder = sstable.compression ? new CompressedSegmentedFile.Builder(null)
	...
	public static SSTableReader open(Descriptor descriptor, Set<Component> components, CFMetaData metadata,
			boolean validate, boolean trackHotness) throws IOException {
		// Minimum components without which we can't do anything
		assert components.contains(Component.DATA) : "Data component is missing for sstable " + descriptor;
		assert !validate
				|| components.contains(Component.PRIMARY_INDEX) : "Primary index component is missing for sstable "
						+ descriptor;

		// For the 3.0+ sstable format, the (misnomed) stats component hold the
		// serialization header which we need to deserialize the sstable content
		assert !descriptor.version.storeRows()
				|| components.contains(Component.STATS) : "Stats component is missing for sstable " + descriptor;
		...
		logger.debug("Opening {} ({} bytes)", descriptor, new File(descriptor.filenameFor(Component.DATA)).length());
		SSTableReader sstable = internalOpen(descriptor, components, metadata, System.currentTimeMillis(),
				statsMetadata, OpenReason.NORMAL, header == null ? null : header.toHeader(metadata));
	...
	public static Collection<SSTableReader> openAll(Set<Map.Entry<Descriptor, Set<Component>>> entries,
		...
		ExecutorService executor = DebuggableThreadPoolExecutor.createWithFixedPoolSize("SSTableBatchOpen",
	...
	public static SSTableReader internalOpen(Descriptor desc, Set<Component> components, CFMetaData metadata,
			SegmentedFile ifile, SegmentedFile dfile, IndexSummary isummary, IFilter bf, long maxDataAge,
			StatsMetadata sstableMetadata, OpenReason openReason, SerializationHeader header) {
		assert desc != null && ifile != null && dfile != null && isummary != null && bf != null
				&& sstableMetadata != null;

		SSTableReader reader = internalOpen(desc, components, metadata, maxDataAge, sstableMetadata, openReason,
				header);
	...
	private static SSTableReader internalOpen(final Descriptor descriptor, Set<Component> components,
			CFMetaData metadata, Long maxDataAge, StatsMetadata sstableMetadata, OpenReason openReason,
	...
	protected SSTableReader(final Descriptor desc, Set<Component> components, CFMetaData metadata, long maxDataAge,
			StatsMetadata sstableMetadata, OpenReason openReason, SerializationHeader header) {
		...
		this.rowIndexEntrySerializer = descriptor.version.getSSTableFormat().getIndexSerializer(metadata, desc.version,
				header);
	}
	...
	public void setupOnline() {
		...
		// e.g. by BulkLoader, which does not initialize the cache. As a kludge,
		// we set up the cache
	...
	private void loadBloomFilter(boolean oldBfHashOrder) throws IOException {
		try (DataInputStream stream = new DataInputStream(
				new BufferedInputStream(new FileInputStream(descriptor.filenameFor(Component.FILTER))))) {
	 ...
	 * filter.
	 * 
	 * @param saveSummaryIfCreated
	 *            for bulk loading purposes, if the summary was absent and
	 *            needed to be built, you can avoid persisting it to disk by
	...
	private void load(boolean recreateBloomFilter, boolean saveSummaryIfCreated) throws IOException {
		try (SegmentedFile.Builder ibuilder = SegmentedFile.getBuilder(DatabaseDescriptor.getIndexAccessMode(), false);
			...
			// details.
			if (ifile != null && !descriptor.version.hasSamplingLevel() && !builtSummary
				...
				try (SegmentedFile.Builder ibuilderRebuild = SegmentedFile
						.getBuilder(DatabaseDescriptor.getIndexAccessMode(), false);
						SegmentedFile.Builder dbuilderRebuild = SegmentedFile
								.getBuilder(DatabaseDescriptor.getDiskAccessMode(), compression)) {
		...
		} catch (Throwable t) { // Because the tidier has not been set-up yet in
								// SSTableReader.open(), we must release the
	 ...
	 * Index.db file.
	 *
	 * @param recreateBloomFilter
	 *            true if recreate bloom filter
	 * @param summaryLoaded
	 *            true if index summary is already loaded and not need to build
	 *            again
	 * @throws IOException
	 */
	private void buildSummary(boolean recreateBloomFilter, boolean summaryLoaded, int samplingLevel)
			throws IOException {
		if (!components.contains(Component.PRIMARY_INDEX))
			return;
		...
		try (RandomAccessReader primaryIndex = RandomAccessReader
				.open(new File(descriptor.filenameFor(Component.PRIMARY_INDEX)))) {
			...
			long estimatedKeys = histogramCount > 0 && !sstableMetadata.estimatedPartitionSize.isOverflowed()
					? histogramCount : estimateRowsFromIndex(primaryIndex); // statistics
																			// is
																			// supposed
																			// to
																			// be
			...
			try (IndexSummaryBuilder summaryBuilder = summaryLoaded ? null
					: new IndexSummaryBuilder(estimatedKeys, metadata.params.minIndexInterval, samplingLevel)) {
				long indexPosition;
	 ...
	 * stored in schema, then Summary.db file will be deleted and this returns
	 * false to rebuild summary.
	 *
	 * @param ibuilder
	 * @param dbuilder
	...
	public boolean loadSummary(SegmentedFile.Builder ibuilder, SegmentedFile.Builder dbuilder) {
		...
		try {
			...
			indexSummary = IndexSummary.serializer.deserialize(iStream, getPartitioner(),
					descriptor.version.hasSamplingLevel(), metadata.params.minIndexInterval,
	 ...
	 * serialization format does not support persisting the sampling level.
	 * 
	...
	private boolean validateSummarySamplingLevel() {
		...
		// for the remainder of the summary). Unfortunately, the first entry to
		// be dropped is the entry at
	...
	// These runnables must NOT be an anonymous or non-static inner class, nor
	// must it retain a reference chain to this reader
	...
	/**
	 * Clone this reader with the provided start and open reason, and set the
	 * clone as replacement.
	 *
	 * @param newFirst
	 *            the first key for the replacement (which can be different from
	 *            the original due to the pre-emptive opening of compaction
	 *            results).
	 * @param reason
	 *            the {@code OpenReason} for the replacement.
	 *
	 * @return the cloned reader. That reader is set as a replacement by the
	 *         method.
	 */
	 ...
	 *            the {@code OpenReason} for the replacement.
	 * @param newSummary
	 *            the index summary for the replacement.
	 *
	 * @return the cloned reader. That reader is set as a replacement by the
	 *         method.
	 */
	private SSTableReader cloneAndReplace(DecoratedKey newFirst, OpenReason reason, IndexSummary newSummary) {
		SSTableReader replacement = internalOpen(descriptor, components, metadata,
				ifile != null ? ifile.sharedCopy() : null, dfile.sharedCopy(), newSummary, bf.sharedCopy(), maxDataAge,
				sstableMetadata, reason, header);
	...
	// runOnClose must NOT be an anonymous or non-static inner class, nor must
	// it retain a reference chain to this reader
	...
	private static class DropPageCache implements Runnable {
		...
		private DropPageCache(SegmentedFile dfile, long dfilePosition, SegmentedFile ifile, long ifilePosition,
				Runnable andThen) {
			this.dfile = dfile;
	 ...
	 * SSTableReader except that a new IndexSummary will be built at the target
	 * samplingLevel. This (original) SSTableReader instance will be marked as
	 * replaced, have its DeletingTask removed, and have its periodic read-meter
	 * sync task cancelled.
	 * 
	 * @param samplingLevel
	 *            the desired sampling level for the index summary on the new
	 *            SSTableReader
	...
	public SSTableReader cloneWithNewSummarySamplingLevel(ColumnFamilyStore parent, int samplingLevel)
		...
		synchronized (tidy.global) {
			...
			// We have to rebuild the summary from the on-disk primary index in
			// three cases:
			...
			if (samplingLevel > indexSummary.getSamplingLevel()
					|| indexSummary.getMinIndexInterval() != minIndexInterval || effectiveInterval > maxIndexInterval) {
				...
				throw new AssertionError(
						"Attempted to clone SSTableReader with the same index summary sampling level and "
	...
	private IndexSummary buildSummaryAtLevel(int newSamplingLevel) throws IOException {
		// we read the positions in a BRAF so we don't have to worry about an
		// entry spanning a mmap boundary.
		RandomAccessReader primaryIndex = RandomAccessReader
				.open(new File(descriptor.filenameFor(Component.PRIMARY_INDEX)));
		try {
			long indexSize = primaryIndex.length();
	...
	/**
	 * Gets the position in the index file to start scanning to find the given
	 * key (at most indexInterval keys away, modulo downsampling of the index
	 * summary). Always returns a {@code value >= 0}
	 */
	 ...
	 *             if the sstable is not compressed
	 */
	...
	public long estimatedKeysForRanges(Collection<Range<Token>> ranges) {
		...
		long estimatedKeys = sampleKeyCount
				* ((long) Downsampling.BASE_SAMPLING_LEVEL * indexSummary.getMinIndexInterval())
				/ indexSummary.getSamplingLevel();
		return Math.max(1, estimatedKeys);
	}

	/**
	 * Returns the number of entries in the IndexSummary. At full sampling, this
	 * is approximately 1/INDEX_INTERVALth of the keys in this SSTable.
	 */
	...
	/**
	 * Returns the approximate number of entries the IndexSummary would contain
	 * if it were at full sampling.
	 */
	...
	private static List<Pair<Integer, Integer>> getSampleIndexesForRanges(IndexSummary summary,
		...
		for (Range<Token> range : Range.normalize(ranges)) {
			...
			int right = Range.isWrapAround(range.left, range.right) ? summary.size() - 1
			...
			if (right < 0) {
				...
				right = (right + 1) * -1;
				if (right == 0)
					// Means the first key is already stricly greater that the
					// right bound
					continue;
				right--;
			}
	...
	public List<Pair<Long, Long>> getPositionsForRanges(Collection<Range<Token>> ranges) {
		...
		for (Range<Token> range : Range.normalize(ranges)) {
			...
			PartitionPosition leftBound = bounds.left.compareTo(first) > 0 ? bounds.left
					: first.getToken().minKeyBound();
			...
			long right = (rightBound.compareTo(last) > 0) ? uncompressedLength()
			...
			assert left < right : String.format("Range=%s openReason=%s first=%s last=%s left=%d right=%d", range,
					openReason, first, last, left, right);
			positions.add(Pair.create(left, right));
		}
		return positions;
	}
	...
	/**
	 * @param key
	 *            The key to apply as the rhs to the given Operator. A 'fake'
	 *            key is allowed to allow key selection by token bounds but only
	 *            if op != * EQ
	 * @param op
	 *            The Operator defining matching keys: the nearest key to the
	 *            target matching the operator wins.
	 * @param updateCacheAndStats
	 *            true if updating stats and cache
	 * @return The index entry corresponding to the key, or null if the key is
	 *         not present
	 */
	protected abstract RowIndexEntry getPosition(PartitionPosition key, Operator op, boolean updateCacheAndStats,
			boolean permitMatchPastLast);

	public abstract UnfilteredRowIterator iterator(DecoratedKey key, Slices slices, ColumnFilter selectedColumns,
			boolean reversed, boolean isForThrift);

	public abstract UnfilteredRowIterator iterator(FileDataInput file, DecoratedKey key, RowIndexEntry indexEntry,
			Slices slices, ColumnFilter selectedColumns, boolean reversed, boolean isForThrift);
	...
	/**
	 * @return The length in bytes of the data for this SSTable. For compressed
	 *         files, this is not the same thing as the on disk size (see
	 *         onDiskLength())
	 */
	 ...
	 *         compressed files, this is not the same thing as the data length
	 *         (see length())
	 */
	...
	/**
	 * Set the value of CRC check chance. The argument supplied is obtained from
	 * the the property of the owning CFS. Called when either the SSTR is
	 * initialized, or the CFS's property is updated via JMX
	 * 
	 ...
	 *
	 * When calling this function, the caller must ensure that the SSTableReader
	 * is not referenced anywhere except for threads holding a reference.
	 *
	 * multiple times is usually buggy (see exceptions in
	 * Tracker.unmarkCompacting and removeOldSSTablesSize).
	 */
	 ...
	 * @return A Scanner for seeking over the rows of the SSTable.
	 */
	public abstract ISSTableScanner getScanner(ColumnFilter columns, DataRange dataRange, RateLimiter limiter,
			boolean isForThrift);
	 ...
	 * localhost currentMilli time). This works in conjunction with maxDataAge
	 * which is an upper bound on the create of data in this sstable.
	 * 
	 * @param age
	 *            The age to compare the maxDataAre of this sstable. Measured in
	 *            millisec since epoc on this host
	 * @return True iff this sstable contains data that's newer than the given
	 *         age parameter.
	 */
	...
	public void createSoftLinks(String movePath) {
		for (Component component : components) {
			File sourceFile=new File(descriptor.filenameFor(component));
			Path target = new File(movePath, sourceFile.getName()).toPath();
			Path link = new File(descriptor.directory, descriptor.filenameFor(component)).toPath();
			try {
	 ...
	 * move SSTables File
	 */
	public void moveFiles(String moveDirectoryPath) {
		for (Component component : components) {
			File sourceFile = new File(descriptor.filenameFor(component));
			File targetFile = new File(moveDirectoryPath, sourceFile.getName());
			try {
	...
	public abstract static class Operator {
		...
		/**
		 * @param comparison
		 *            The result of a call to compare/compareTo, with the
		 *            desired field on the rhs.
	...
	/**
	 * sstable contains no tombstones if maxLocalDeletionTime ==
	 * Integer.MAX_VALUE
	 */
	public boolean hasTombstones() {
		// sstable contains no tombstone if minLocalDeletionTime is still set to
		// the default value Integer.MAX_VALUE
	...
	public int getAvgColumnSetPerRow() {
		return sstableMetadata.totalRows < 0 ? -1
				: (sstableMetadata.totalRows == 0 ? 0
						: (int) (sstableMetadata.totalColumnsSet / sstableMetadata.totalRows));
	}
	 ...
	 *
	 * Called after level is changed on sstable, for example if the sstable is
	 * dropped to L0
	 *
	...
	public void reloadSSTableMetadata() throws IOException {
		this.sstableMetadata = (StatsMetadata) descriptor.getMetadataSerializer().deserialize(descriptor,
				MetadataType.STATS);
	}
	 ...
	 * @return last modified time for given component. 0 if given component does
	 *         not exist or IO error occurs.
	 */
	...
	/**
	 * Increment the total row read count and read rate for this SSTable. This
	 * should not be incremented for range slice queries, row cache hits, or
	 * non-query reads, like compaction.
	 */
	...
	/**
	 * Checks if this sstable can overlap with another one based on the min/man
	 * clustering values. If this methods return false, we're guarantee that
	 * {@code this} and {@code other} have no overlapping data, i.e. no cells to
	 * reconcile.
	 */
	 ...
	 * sstable with MOVED_START for example), but there can only be one
	 * GlobalTidy for one single logical sstable.
	 *
	 * When the InstanceTidier cleansup, it releases its reference to its
	 * GlobalTidy; when all InstanceTidiers for that type have run, the
	 * GlobalTidy cleans up.
	 */
	...
	/**
	 * One instance per logical sstable. This both tracks shared cleanup and
	 * some shared state related to the sstable's lifecycle.
	 *
	 * All InstanceTidiers, on setup(), ask the static get() method for their
	 * shared state, and stash a reference to it to be released when they are.
	 * Once all such references are released, this shared tidy will be
	 * performed.
	 */
	static final class GlobalTidy implements Tidy {
		...
		// the readMeter that is shared between all instances of the sstable,
		// and can be overridden in all of them
		// at once also, for testing purposes
		private RestorableMeter readMeter;
		// the scheduled persistence of the readMeter, that we will cancel once
		// all instances of this logical
		// sstable have been released
		private WeakReference<ScheduledFuture<?>> readMeterSyncFuture = NULL;
		// shared state managing if the logical sstable has been compacted; this
		// is used in cleanup
	...
	public static abstract class Factory {
		public abstract SSTableReader open(final Descriptor descriptor, Set<Component> components, CFMetaData metadata,
...
>>>>>>> cc0ab8f733eef63ed0eaea30cc6f471b467c3ec5
}
